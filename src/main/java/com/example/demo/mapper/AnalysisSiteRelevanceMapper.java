...
public interface AnalysisSiteRelevanceMapper {
     ...
     * @date 2020/4/24 14:39
     */
    @Select({" select  SUM(use_num) as value ,type_name as name from lan_data  " +
            "where create_at>=#{start_time} and create_at<=#{end_time}" +
            "GROUP BY type_name  "})
    List<HashMap> SelectDataSiteRange(@Param("start_time") String start_time, @Param("end_time") String end_time);
